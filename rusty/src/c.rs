...
pub struct EntityVTable {
    pub on_tick: *const extern "C" fn(*const c_void, f32),
    pub on_render: *const extern "C" fn(*const c_void),
    pub on_setup: *const extern "C" fn(*const c_void, u32),
    pub destructor: *const extern "C" fn(*const c_void),
    pub serialize: *const extern "C" fn(*const c_void, arena: *mut c_funcs::Arena) -> CVec<u8>,
}
#[allow(unused)]
macro_rules! setup_vtable {
    ($T:ident, $name:ident, $destructor:ident, $setup_fn_name:ident) => {
        static $name: EntityVTable = EntityVTable {
            on_tick: $T::on_tick as OnTick,
            on_render: $T::on_render as OnRender,
            on_setup: $T::on_setup as OnSetup,
            destructor: $destructor as Destructor,
            serialize: $T::serialize as Serializer,
        };
        pub fn $setup_fn_name() -> CEntity {
            CEntity {
                vtable: &$name as *const EntityVTable,
                self_id: 0,
            }
        }
    };
...
pub type OnTick = *const extern "C" fn(*const c_void, f32);
pub type OnRender = *const extern "C" fn(*const c_void);
pub type OnSetup = *const extern "C" fn(*const c_void, u32);
pub type Destructor = *const extern "C" fn(*const c_void);
pub type Serializer = *const extern "C" fn(*const c_void, arena: *mut c_funcs::Arena) -> CVec<u8>;
#[allow(unused)]
pub trait Entity {
...
impl Entity for CEntity {
    fn on_tick(&mut self, dt: f32) {
        unsafe { (*((*self.vtable).on_tick))(self as *mut Self as *mut c_void, dt) }
    }
    fn on_render(&self) {
        unsafe { (*((*self.vtable).on_render))(self as *const Self as *mut c_void) }
    }
    fn on_setup(&mut self, id: u32) {
        unsafe { (*((*self.vtable).on_setup))(self as *const Self as *mut c_void, id) }
    }
    fn destructor(&mut self) {
        unsafe { (*((*self.vtable).destructor))(self as *const Self as *mut c_void) }
    }
}
#[repr(C)]
#[derive(Clone, Copy)]
...
pub mod c_funcs {
    ...
    extern "C" {
        ...
        pub fn call_event(
            id: u32,
            event: *const extern "C" fn(this: *mut CEntity, args: *const c_void),
            args: *const c_void,
        );
    }
...
pub fn remove_transform_comp(id: u32) -> bool {
    unsafe { c_funcs::remove_model_comp(id) }
}

pub fn set_physics_comp(id: u32, phys: PhysicsComp) -> bool {
    unsafe { c_funcs::set_physics_comp(id, phys) }
}
...
pub fn remove_physics_comp(id: u32) -> bool {
    unsafe { c_funcs::remove_physics_comp(id) }
}

pub fn set_model_comp(id: u32, model: ModelComp) -> bool {
    unsafe { c_funcs::set_model_comp(id, model) }
}
...
pub fn call_event<T: Entity>(id: u32, func: Box<dyn Fn(&mut T)>) {
    ...
    unsafe {
        func(
            (get_entity(&id).expect("msg") as *mut CEntity as *mut T)
                .as_mut()
                .expect("msg"),
        );
        let arrrrg = c_funcs::tmp_alloc(size_of::<Box<dyn Fn(&mut dyn Entity)>>())
            as *mut Box<dyn Fn(&mut T)>;
        memcpy(
            arrrrg as *mut c_void,
            func.as_ref() as *const dyn Fn(&mut T) as *mut c_void,
            16,
        );
        let _ = ManuallyDrop::new(func);
        c_funcs::call_event(
            id,
            thunk::<T> as *const extern "C" fn(*mut CEntity, *const c_void),
            arrrrg as *const c_void,
        );
    }
}
